...
def _kde_log(values, grid, np):
    """Normalised kernel density over ``grid`` (log space), or None if n < 8.

    Below 8 animals a KDE is a picture of the bandwidth, not of the data, so we
    ...
    if len(v) < 8:
        return None
    lv = np.log(v)
    # Silverman, eased up for small samples so a 23-animal plate does not
    # sprout spurious modes next to a 639-animal one.
    ...
    dens = dens / np.exp(grid)          # log-density -> size-density
    m = dens.max()
...
def build_size_payload(soft_csv: Path, per_image_rows: list[dict],
                       write_log: Callable[[str], None]) -> Optional[dict]:
    """Group every detection's size_px by (condition, timepoint) and summarise.
    ...
    all_sizes = np.array([s for v in rows_by_group.values() for s in v],
                         dtype=float)
    lo = float(np.log(max(float(np.quantile(all_sizes, 0.002)), 1.0)))
    ...
    write_log(f"Body size: {n_total:,} animals in {len(groups)} group(s), "
              ...
              f"{edges_px[0]:.1f}–{edges_px[-1]:.1f} px, {_GRID_N - 1} bins "
              "(log-spaced; size_histogram lists them).")
    return {
